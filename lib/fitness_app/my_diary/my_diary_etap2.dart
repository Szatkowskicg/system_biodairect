...
class MyDiaryEtap2 extends StatefulWidget {
  const MyDiaryEtap2({Key? key, this.animationController, this.countDay}) : super(key: key);

  final String? countDay;
  final AnimationController? animationController;
  @override
  _MyDiaryEtap2State createState() => _MyDiaryEtap2State();
}

class _MyDiaryEtap2State extends State<MyDiaryEtap2>
 ...
 void loadData() {
      ref.child('users').child(uid).child('dailyInfo').child(todayDate).onValue.listen((event) {
        final dailyInfo = DailyInfo.fromMap(event.snapshot.value);
        setState(() {
          if (event.snapshot.value != null){
            scaleVar = dailyInfo.scale;
            // addAllListData();
          }
        });
      });
  ...
  Widget getMainListViewUI() {
    return StreamBuilder(
      stream: ref.child('users').child(uid).child('dailyInfo').child(todayDate).onValue,
